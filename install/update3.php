...

{
	...
	if ($noEtape == 8) { // création des meta tables
	...
	} elseif ($noEtape == 9) { // création des tables nouvelles
		...
		$noEtape = 10;
		setMaintenance($noEtape);
	} elseif ($noEtape == 10) { // maise a mesure des champs selon l'ancienne base

		foreach ($tables as $table)
			allongeZoneTables(EA_DB . "_" . $table, $logtxt);
		$noEtape = 11;
		setMaintenance($noEtape);
	} elseif ($noEtape >= 11 and $noEtape <= 14) { // Transfert des données ACTES
		$code = $codes[$noEtape];
		...
		$nid = nextID($code);
		while ($nid >= 0 and (time() - $T0 < $Max_time)) { // on continue un paquet de lignes
	...
	elseif ($noEtape == 15) { // Transfert des données USERS
		$code = $codes[$noEtape];
...
if (mysql_num_rows($res) > 0) {
	...
	$login = 2;
if ($login == 3) { // On peut déjà utilser V3 normal
	$userlogin = "";
	$userlevel = logonok(9);
	if ($userlevel < 9) {
		login($root);
