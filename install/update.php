...
$old_param = (defined("SITENAME") or defined("SITEURL") or defined("PUBLIC_LEVEL"));
// recupération des variables éventuelles
...
if ($result) {
				...
				if ($line[0] == "#") {
					...
					switch ($cond[0]) {
						case "TYPE":
							...
							break;
						case "RENAME":
							$res = mysql_query("SHOW COLUMNS FROM " . $cond[1] . " LIKE '" . $cond[2] . "';");
							if (mysql_num_rows($res) > 0) {
								$row = mysql_fetch_array($res);
						...
						case "ADDCOL":
							...
							break;
						case "INDEX":
							$res = mysql_query("SHOW INDEX FROM " . $cond[1] . "; ");
							...
							if ($doline == 1) {
								$res = mysql_query("select count(*) as NBRE from " . $cond[1] . "; ");
								$row = mysql_fetch_array($res);
								$totfiches = $row[0];
								if ((val_var_mysql('wait_timeout') < 60 or val_var_mysql('net_write_timeout') < 60) and $totfiches > 50000) {
									...
									echo "<p>La commande d'indexation non exacutée est la suivante :</p>";
									$doline = 2;
							...
							break;
						case "ADDTABLE":
							$res = mysql_query("SHOW TABLES LIKE '" . $cond[1] . "';");
							if (mysql_num_rows($res) == 0) {
								$doline = 1;
							}
							break;
	...
	if ($doline < 9) {
		...
		if ($old_param) {
			echo '<h1><font color="#FF0000">ATTENTION</font></h1>';
			echo '<p><font color="#FF0000">';
			if ($par_add > 0)	echo "Les paramètres de configuration ont été copiés dans la base de données.<br />";
			echo "Vous DEVEZ à présent supprimer du fichier <b>_config/connect.inc.php</b> les paramètres avec noms en majuscules.<br />";
			echo "Vous DEVEZ par contre <b>conserver les 4 variables " . '$dbaddr, $dbuser, $dbpass, $dbname' . "</b> qui définissent l'accès à la base de données.<br />";
			echo "Sinon, la gestion interactive des paramètres sera impossible.";
			echo '</font></h1>';
		}
