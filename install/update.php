...
{ //list($autokey, $continue, $nb_refresh, $reload_deja, $trace) = reload_check_autokey($xaction, $tokenfile, $autokey, $nombre_total, $param_script);
	...
	unset($queries['autokey']);
	unset($queries['action']);
	$param_script = '&' . http_build_query($queries, null, '&amp;');
	...
	switch ($xaction) {
		...
		case 'go':
			...
			if (!empty($autokey)) { // récupération des valeurs dans le fichier
				$x = false;
...
if ($result) {
	...
	for ($num_line = 0; $num_line < $boucle_max; $num_line++) {
		...
		if (strpos($line, '//Depuis-' . EA_VERSION) !== false) {
			...
			if ($line == "" || $line == "\n" || !$a_traiter || !in_array(substr($line, 0, 1), array('#', '@', '>'))) { // passe les lignes vides, celles hors scope de la version (Depuis-VVV), et celles non traitées
				continue;
			} else {
				...
				if ($line[0] == ">") {
					...
					if ($doline == 1)  // executer vraiment
					{
						...
						if ($res = EA_sql_query($reqmaj)) {
							...
							//		$pas_a_pas = true; // Force le pas à pas
							//	}
							if ((strpos($line, 'UPDATE ') !== false) and ($i !== false) and (strpos($line, ' SELECT ') !== false)) { // C'est un UPDATE avec SELECT et LIMIT
								...
								if ($res = EA_sql_query($x)) {
									$row = EA_sql_fetch_array($res);
									if ($row[0] > 0) { // Il faut refaire l'UPDATE
