...
// Attn si on choisi ADDR modifier dans la génération lignes_fichier_config_ancien
// $Ref_Serveur = str_replace(array(':','.'), '-' , $_SERVER['SERVER_ADDR']);
...
define ('CONFIG_FILENAME_NEW', 'BD-'. $Ref_Serveur . '-connect.inc.php');
define ('Old_CONFIG_FILENAME_TEXTE', "'BD-'. \$Ref_Serveur . '-connect.inc.php'");
...
if ($phase != 5) {
    ...
    if (file_exists( CONFIG_DIRNAME . CONFIG_FILENAME_NEW ) ) {
        ...
        if ( $texte_temoin !== '' ) {
            ...
            $lignes_fichier_config_ancien[] = "\$Ref_Serveur = str_replace(array(':','.'), '-' , \$_SERVER['SERVER_NAME']);" . "\n";
			$lignes_fichier_config_ancien[] = "if (strtoupper( substr( \$Ref_Serveur, 0 , 4) ) == 'WWW-') \$Ref_Serveur = substr( \$Ref_Serveur, 4);" . "\n";
