...
foreach ($GLOBALS['Type_Table'] as $k => $v) {
	...
	if ($result = EA_sql_query($requete)) {
		...
		while ($row = EA_sql_fetch_assoc($result)) {
			...
			if ($row['Comment'] == 'disabled') { // Faire : "ALTER TABLE latable ENABLE KEYS"
				$defaut_index = true;
				echo '<p><font color="#FF0000"><b>La table ' . $v . ' n\'a plus l\'index ' . $row['Key_name'] . ' actif, des lenteurs sont probables.</b></font></p>';
			}
		}
		sort($t);
		if ($tableau_cle[$k] != $t) { // Faire : "ALTER TABLE latable ADD INDEX `nom_index` (`ref_colonne`)" (voir la création des tables install/creetables.sql)
