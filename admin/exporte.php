...
if ($xaction == 'go') {
	...
	if ((empty($TypeActes) or ($TypeActes == 'X'))  or empty($Destin) or (empty($Commune))) {
		init_page();
		menu_datas('B');
		if (empty($TypeActes) or ($TypeActes == 'X')) msg('Vous devez préciser le type des actes');
		if (empty($Destin))    msg("Vous devez indiquer la destination de l'export");
		if (empty($Commune)) msg('Vous devez préciser une commune.');
		$missingargs = true;
	}
	if (empty($autokey))
...
if (!$missingargs) {
	...
	if ($continue == 0) // fin d'une chaine automatique
	{
		...
		$oktype = true;
		if ($Destin == 'B') $olddepos = 0;
		else $olddepos = getparam('olddepos', 0);
		$params = array(
			'xtdiv' => $xtdiv,
			'userlevel' => $userlevel,
			'userid' => $userid,
		...
		if (($TypeActes == 'V') and ($soustype != '')) $extstype = "_" . mb_substr($xtdiv, 0, 4);
		...
		if ($xaction == 'go') {
			...
			*/
			if ($nbdocs == 0) {
				...
				$request = "select * from " . $table .
					" where " . $condcom . $condad . $condaf . $conddep . $condtdiv;
				if ($Destin <> "B") $request .= ' order by ladate, nom, pre';
				if (MAX_SELECT_REC > 0)
