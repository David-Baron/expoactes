...
if (getparam('action') == 'submitted')
  ...
  if ($autopw)
	  ...
	  if ($id==-1 or !empty($lepassw)) 
			...
			$pw = "";
	$res=mysql_query("SELECT * FROM ".EA_UDB."_user3 WHERE login='".sql_quote($lelogin)."' and id <> ".$leid,$u_db);
	...
	if (TEST_EMAIL_UNIC==1)
		{
		$res=mysql_query("SELECT * FROM ".EA_UDB."_user3 WHERE email='".sql_quote(getparam('email'))."' and id <> ".$leid,$u_db);
		if (mysql_num_rows($res)!=0)
			{
			$row = mysql_fetch_array($res);
