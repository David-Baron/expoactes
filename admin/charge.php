...
if (getparam('action') == 'submitted') 	// Lancement d'une opération de chargement ou de restauration
    ...
    //	{ print '<pre>TOF : ';  print_r($vals); echo '</pre>'; }
			...
			elseif ($vals[1]<>$autokey)
				...
				$file_no=$vals[3];
				$uploadfile=$vals[4];
				if ($Origine=="B") $numfile = mb_substr($uploadfile,-7,3);
				else $numfile = 0;
...
if($ok)
	if (isset($continue) and $continue==0)
        ...
        { // par ligne
			my_flush(200);
            if  (time()-$T0>$Max_time) break ; // Le temps limite d'un script est atteint, on sort de la boucle pour relance
			/*
			$TX = time();
			if ($T1<>$TX)
				{
...
//	echo '<pre>'; print_r($mdb); echo '</pre>';
			...
			if ($oktype==true and $line_num >= $passlign)
				{	// --------- Traitement ----------
				$data = acte2data($acte,$moderestore);
				...
				elseif (count($acte)<$minzones)
					...
					if ($TypeActes=="V") {$decal=2;} else {$decal=0;}
					$nom     = trim($data["NOM"]);
					$pre     = trim($data["PRE"]);
					$idnim   = $data["IDNIM"];
					if ( trim($idnim) == '') { $idnim = $data["IDNIM"] = 0;}
					...
					if ($moderestore)
						...
						if (($data["P_NOM"].$data["P_PRE"].$data["M_NOM"].$data["M_NOM"] =="") and ($Filiation==1))
						 ...
						 elseif (($AVerifier==1) and (mb_strpos($acte[8],"RIF") > 0))
