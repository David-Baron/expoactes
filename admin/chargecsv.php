...
//		{ // Format LIBRE
		...
		if ( ($submit == 'L') and (getparam('ModeleL') !== '') )
		  { //Chargement d'un fichier modèle
		  ...
		  // chargement effectif
			...
			if ($TypeActes == 'V' and $typedoc == "" and empty($_REQUEST[$zonelibelle]))
				...
				foreach ($csv as $line_num => $line)
					{ // par ligne
					my_flush(200);
					if ($line_num >= getparam('passlign') and (time()-$T0<$Max_time))
						{ // ligne à traiter
						$curr_line=$line_num;
						$listzone[0] = "";
						$listzone[1] = "";
						...
						if (!empty($Filtre) and !comparer($acte[$Filtre-1],$Compare,$Condition))
							...
							foreach($mdb as $zone)
								{
								if (array_key_exists('ZID'.$i,$_REQUEST) and $zone['ZID']==$_REQUEST['ZID'.$i])
									{ // traiter ce champ
							...
							*/
							...
							elseif ($data["COMMUNE"] !== '' )
								$commune = $data["COMMUNE"]; // Attention il faut conserver $commune cohérent pour les relances
							...
							if (($SuprPatVid==1) and ($nom ==""))
							 ...
							 elseif ($data["COMMUNE"]=="")
								...
								{
								$cptign++;
								if ($logKo==1) echo $log."PAS DE COMMUNE (".$curr_line.") -> Ignoré";
								}
							 ...
							 elseif (($AnneeVide==1) and ($MauvaiseAnnee == 1))
								...
								if ($nom=="") $nom = "N";
								//if ($cnom=="") $cnom = "N";
								if ($TypeActes=="M" and $SuprRedon==1)
									{  // inversion éventuelle des mariages
									$inversion = false;
...
//{ print '<pre>';  echo $cookname." : ".$_COOKIE[$cookname].'<br>'; print_r($presets); echo '</pre>'; }
			...
			if ($submit <> 'C')
				{
				if (getparam('ModeleL') !== '')
				{ // Affiche le nom du modèle chargé
...
//{ print '<pre>';  print_r($mdb); echo '</pre>'; }
			...
			if ( ($submit == 'S') and (getparam('ModeleS') !== '') )
				{ // Sauvegarde modèle
			...
			if ($submit == 'V')  // Voir un exemple
				...
				if (isset($_REQUEST['nofiche']))
				  ...
				  $nofiche = 1;
				  if (!isset($csv[$nofiche]))
				    { // On arrive au bout du fichier on repart sur la 1ère ligne
					  $nofiche = 1;
				    }
				 $line=$csv[$nofiche];
				 if (!$isUTF8)
				...
				foreach($mdb as $zone)
					...
					if (array_key_exists('ZID'.$i,$_REQUEST) and $zone['ZID']==$_REQUEST['ZID'.$i])
						...
						if ($zone['OBLIG']== 'Y' and $_REQUEST['Zone'.$i] == 0)
							{ // zone obligatoire
