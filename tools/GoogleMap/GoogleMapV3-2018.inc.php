 ...
 ***  - Ajout des �l�ments permettant de r�cup�rer les coordonn�es d'un marker "draggable"
 ***
 * 
...
class GoogleMapAPI
{
    ...
    /**
     * id of form variable for returning LON after dragging the marker
     *
     * @var string
     */
    var $form_lon = 'lon';
    ...
    /**
     * id of form variable for returning LAT after dragging the marker
     *
     * @var string
     */
    var $form_lat = 'lat';
    ...
    /**
     * determines whether or not to use the Marker is draggable
     */
    var $marker_draggable = false;

    /**
     * function to enable draggable marker  (supposed to be alone)
     */
    function enableMarkerDraggable()
    {
        $this->marker_draggable = true;
    }

    /**
     * function to disable draggable marker 
     */
    function disableMarkerDraggable()
    {
        $this->marker_draggable = false;
    }
    ...
    function getCreateMarkerJS()
    {
        $_output = "
    	   function createMarker(map, point, title, html, icon, icon_shadow, sidebar_id, openers){
			    var marker_options = {
			        position: point,
							draggable: " . (($this->marker_draggable) ? "true" : "false") . ",
			        map: map,
			        title: title};  
			    ...
			    if(html!=''){
					...
					";
        /******  ADLC begin 2 *****/
        if ($this->marker_draggable)
            $_output .= " google.maps.event.addListener(new_marker, 'dragend', outputCoords); ";
        /****** ADLC end *****/
        $_output .= " return new_marker;  
				}
				";
        /******  ADLC begin 3 *****/
        if ($this->marker_draggable) {
            $_output .= "
					function outputCoords(position) {
						document.getElementById('" . $this->form_lon . "').value=position.latLng.lng();
						document.getElementById('" . $this->form_lat . "').value=position.latLng.lat();
						} ";
        }
        /****** ADLC end *****/
        return $_output;
