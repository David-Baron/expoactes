...
function getparam($name,$default="",$allow_sql=0) // initialise proprement une variable avec le contenu d'un paramètre facultatif et filtrage anti injection de code
  ...
  if (isset($_REQUEST[$name]))
      ...
      if ( isset($GLOBALS['userlevel']) and $GLOBALS['userlevel'] >= 6 )
      { // Retire UNION de la liste des mots à risque d'injection quand on a des droits suffisants
...
function my_ob_start_affichage_continu()
	{ // Remplace les appels ob_start(); ob_implicit_flush(1);
