...
/**
 * pour détection des optimisations à faire
 */
...
/**
 * retourne la position de $petit dans $grand ou -1 si non présent
 */
function isin($grand, $petit, $debut = 0)
{
	...
	if (is_dir($dir)) {
		if ($dh = opendir($dir)) {
			while (($file = readdir($dh)) !== false) {
				if (strtoupper(mb_substr($file, - ($lext))) == $extup) $files[] = $dir . $file;
			}
		}
	}
	sort($files);
...
/**
 * is bool alors vrai sinon faux
 */
function iif($bool, $vrai, $faux)
{
	if ($bool) return $vrai;
	return $faux;
}
...
/**
 * pour mettre au format entier avec séparateur
 */
function entier($nbre)
{
	return number_format($nbre, 0, ',', '.');
}

/**
 * explose une chaines dans un tableau et complete si besoin les cases manquantes
 */
function ads_explode($chaine, $separ, $nbre)
{
...
/**
 * pour précéder un entier de 0  (ex 006)
 */
function zeros($nbre, $chiffres)
{
	return mb_substr('0000000000' . $nbre, -$chiffres, $chiffres);
}

/**
 * retourne vrai si aucune quote dans le texte
 */
function sans_quote($texte)
{
...
/**
 * transforme en lien actif les nom de fichier image et les URL rencontrés
 * mode : 0 = ne rien faire, >0 activer les lien : 1 = séparés par des , ou des blancs ou  2 = séparés par des ;
 */
function linkifie($texte, $mode)
{
	...
	// $URL_JPG = "http://site1/;000>http://site2/;RFT_>http://site3/";
	$uptexte = strtoupper($texte);
	if (multisin($uptexte, $listExtImages) < 0 && isin($uptexte, 'HTTP://') < 0 and isin($uptexte, 'HTTPS://') < 0) {
		...
		foreach ($lbase as $code) {
			$pl = isin($code, '>>');
			if ($pl > 0) {
				$prefix[] = trim(strtoupper(mb_substr($code, 0, $pl)));
		...
		foreach ($list as $image) {
			if (isin(strtoupper($image), 'IFRAME') == -1) {
				// Traitement des URL directes HTTP://
				$l = isin(strtoupper($image), 'HTTP://');
				if ($l == -1)
					$l = isin(strtoupper($image), 'HTTPS://');
				if ($l >= 0) {
					...
					$uptexte = trim(strtoupper($image));
					$l = multisin($uptexte, $listExtImages);
					if ($l >= 0) {
						$lesite = "";
						for ($ii = 0; $ii < count($prefix); $ii++) {
							if (mb_substr($uptexte, 0, strlen($prefix[$ii])) == $prefix[$ii]) {
								$lesite = $siturl[$ii];
								if ($modurl[$ii] == "R")
									$image = mb_substr(trim($image), strlen($prefix[$ii]));
								break;
							}
						}
						...
						$ipublic = true;
						if (isin(strtoupper($urlimage), 'PRIVE') > -1) {
							$ipublic = false;
							...
							if (($userlogin == "") and (JPG_SI_LOGIN == 1)) {
							$cpt++;
							$result .= ' Image' . $cpt . ' privée';	// On ne montre pas car login obligatoire
							$ipublic = false;
						}

						if ($ipublic) {
							$cpt++;
							$result .= ' <a href="' . $urlimage . '" target="_blank">Image' . $cpt . '</a> ';
						}
					} else
						$result .= ' ' . $image;
				}
...
/**
 * pour retirer les magic quotes s il y en a !! sur REQUEST et $_COOKIE 
 * @Deprecated from PHP 7.4 and useless since 5.4)
 */
function dequote_magic_quotes()
{
...
/**
 * ajoute and si condition non vide
 */
function sql_and($cond)
{
...
/**
 * pour passer texte a MySQL en escapant les ' " \ ...
 */
function sql_quote($texte)
{
...
/**
 * ajoute , si liste non vide
 */
function sql_virgule($cond, $add)
{
...
function explode_csv($line)
{
	...
	for ($i = 0; $i < $l; $i++) {
		if ($line[$i] == '"') {
			if ($guill) $guill = false; // TODO: What the f*** is that????
			else $guill = true;
 ...
 */
function is__writable($path, $show = true)
{
...
/**
 * fonction de connexion à une DB
 */
function con_une_db($ladbaddr, $ladbuser, $ladbpass, $ladbname, $show = false)
{
	global $dbok;
	if (function_exists('EA_sql_connect')) {
		$dblink = @EA_sql_connect("$ladbaddr", "$ladbuser", "$ladbpass", true);
	} else {
		$dblink = @mysql_connect("$ladbaddr", "$ladbuser", "$ladbpass", true);
	}
	if ($dblink) {
...
/**
 * fonction de connexion des DB
 */
function con_db($show = false)
{
	...
	if (isset($udbaddr, $udbuser, $udbpass, $udbname)) {
		...
		$a_db = con_une_db($dbaddr, $dbuser, $dbpass, $dbname, $show);
	} else {
		if ($show) {
...
/**
 * @Deprecated Use password_hash and password_verify instead
 */
...
/**
 * @Deprecated Use php email validator instead
 */
