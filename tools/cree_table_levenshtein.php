...
//								) ENGINE=MyISAM  DEFAULT CHARSET=latin1 COLLATE=".$COLLATION.";";
	  ...
	  $result = mysql_query($request) or die('Erreur SQL creation !'.$sql.'<br>'.mysql_error());
...
//$T5 = time(); 
	  ...
	  $result = mysql_query($request)or die('Erreur SQL !'.$sql.'<br>'.mysql_error());
...
//		echo '<p>Durée requete temp '.$hf.'  tot : '.$nb.'   : '.(time()-$T5).' sec.</p>'."\n";
	  ...
	  while ($ligne = mysql_fetch_row($result))
		...
		$k = levenshtein(strtoupper($xacht),strtoupper($ligne[0])); 
	      if ($k < $dm)
         {
         if ($hf == "H") {	$request1 ="insert IGNORE into ".EA_DB."_".$ip_adr_trait."_h (nomlev,disth) values ('".sql_quote($ligne[0])."',".$k." )";}
         if ($hf == "F") {	$request1 ="insert IGNORE into ".EA_DB."_".$ip_adr_trait."_f (nomlev,distf) values ('".sql_quote($ligne[0])."',".$k." )";}         
         if ($hf == "D") {	$request1 ="insert IGNORE into ".EA_DB."_".$ip_adr_trait."_d (nomlev,distd) values ('".sql_quote($ligne[0])."',".$k." )";}  
         if ($hf == "N") {	$request1 ="insert IGNORE into ".EA_DB."_".$ip_adr_trait."_n (nomlev,distn) values ('".sql_quote($ligne[0])."',".$k." )";}          

				 $result1 = mysql_query($request1) or die('Erreur SQL insertion !'.$sql.'<br>'.mysql_error());	
			//$i++;
			}
			}
