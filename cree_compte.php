...
if (getparam('action') == 'submitted')
	...
	$pw = $lepassw;	
	$res=mysql_query("SELECT * FROM ".EA_UDB."_user3 WHERE login='".sql_quote($lelogin)."'",$u_db);
	...
	$res=mysql_query("SELECT * FROM ".EA_UDB."_user3 WHERE email='".sql_quote($email)."'",$u_db);
	if (mysql_num_rows($res)!=0)
	  {
	  $row = mysql_fetch_array($res);
	...
	if ($ok)
		 ...
		 if  ($result = mysql_query($reqmaj,$u_db))
			...
			$urlvalid = "http://".$_SERVER['HTTP_HOST'].$root."/activer_compte.php?login=".$lelogin."&key=".$clevalid.$crlf.$crlf;
			$urlsite = "http://".$_SERVER['HTTP_HOST'].$root."/index.php";
