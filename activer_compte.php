...
$res=mysql_query("SELECT * FROM ".EA_UDB."_user3 WHERE login='".sql_quote($_REQUEST['login'])
  ...
  $login=$row['login']; 
	...
	if  ($result = mysql_query($reqmaj,$u_db))
		...
		if (USER_AUTO_DEF==1)
			...
			$message .= "http://".$_SERVER['HTTP_HOST'].$root."/admin/approuver_compte.php?id=".$id."&action=OK".$crlf;
			$message .= "OU ".$crlf; 
			$message .= "Vous pouvez REFUSER cet acces avec le lien suivant : ".$crlf; 
			$message .= "http://".$_SERVER['HTTP_HOST'].$root."/admin/approuver_compte.php?id=".$id."&action=KO".$crlf;
