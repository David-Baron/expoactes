...
$result = mysql_query($request) or die('Erreur SQL duplication !'.$sql.'<br>'.mysql_error());
$request = "INSERT INTO ".EA_DB."_".$nom." SELECT * FROM ".EA_DB."_".$original.";";
$result = mysql_query($request) or die('Erreur SQL recopie !'.$sql.'<br>'.mysql_error());
...
if (current_user_solde()>0 or RECH_ZERO_PTS==1)
	...
	if ($xcomp =="Z"){$dm='aucune différence';}
	if ($xcomp =="U"){$dm='une différence';}
	if ($xcomp =="D"){$dm='deux différences';}
	if ($xcomp =="T"){$dm='trois différences';}
	if ($xcomp =="Q"){$dm='quatre différences';}
	if ($xcomp =="C"){$dm='cinq différences';}
	if ($xmax !='')
		{
		if ($xmin !='') 
			{
	...
	$mes = 'du nom <b>'.strtoupper($xach).'</b> avec '.$dm.' sur les ';
		...
		if ((time()-$T0>=$Max_time) or ($fin_ok != 'ok'))
		 ...
		 else
			{
					...
					$result = mysql_query($request)or die('Erreur SQL requete générale!'.$sql.'<br>'.mysql_error());
